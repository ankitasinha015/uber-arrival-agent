...
Status: SHIPPED — see **As-built** below for the accurate current state
Mode: Builder (portfolio / interview artifact)

## As-built (what actually shipped)

The single-scenario "streaming reasoning" demo planned below grew into a **trip
concierge**: one agent that watches a whole trip across several moments, deployed live.
Read this section for the current state, then treat everything after it as the original
planning record, preserved as-is.

**Product**
- **Assistant-thread UI**, not the streaming-reasoning panel specced in the Design System
  section below. A persona login, then a calm thread of agent beats and inline action cards
  you tap to approve. The restrained design tokens (Inter, near-monochrome, one accent)
  carried over; the specific screen layouts did not.
- **Five personas, five cities** (Chicago, New York, LA, San Francisco, Berlin/international),
  each exercising a different facet: full hand-holding, memory-trimmed restraint, silence,
  flight-change recovery, taste-led ordering. See `docs/PERSONAS.md`.
- **Multiple moments, one engine:** a departure nudge, delay/flight-change (the agent
  auto-notifies the hotel, then trip-syncs every arrival-coupled booking), and arrival
  (welcome, honest exit-time, dinner to the room).
- **Intensity dial** (HIGH/LOW/NONE) scales the whole action list to the traveler and moment.
- Still the core, exactly as planned: the wait rule (a naive baseline orders at 00:32, the
  agent waits until 01:12) and the choice-set design. See `docs/CASE_STUDY.md`.

**Data (where the plan below is most out of date)**
- Traveler and order data live in **ChromaDB**, run in Docker (the native build segfaults on
  this Windows machine, so the app talks to it over HTTP). Two collections: synthetic
  travelers, and synthetic Uber Eats order history. Dinner taste ranking is cuisine-frequency
  derived from that history, not a hardcoded list. See `docs/DATA_STORE.md`.
- A **confidence-gated vector classifier** (MiniLM embeddings against per-cuisine exemplars,
  0.65 threshold) replaced the regex cuisine aliases, tuned to zero out-of-set false badges on
  a 40-case stress test.
- The direct-cosine `taste.py` store described below still exists, but now only backs the
  framework-comparison adapters (raw / langgraph / naive). Its "why not Chroma" analysis
  applies to that path, not to the concierge, which does use Chroma.

**Evaluation (not in the original plan)**
- A full eval program: binary evaluators, an LLM judge validated at 100% true-positive and
  true-negative rate on a fresh 40-case held-out set (train/dev/test split + bias correction),
  a RAG retrieval eval, and the classifier stress-test. It caught real bugs. See `evals/README.md`.

**Deploy**
- **Hugging Face Spaces**, not Fly.io. Live Foursquare/Mapbox calls per traveler, no bundled
  data. `fly.toml` is kept as an alternative. See `docs/DEPLOY_HF.md`.

---

_Everything below is the original /office-hours + CEO/design planning record, preserved for
the interview story. See **As-built** above for what actually shipped._
